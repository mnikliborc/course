...
--parseChar :: Input -> ParseResult Char
--parseChar Nil = ErrorResult UnexpectedEof
--parseChar (h:.t) = Result t h
...
satisfy p =
  --P (\i -> case i of 
  --    Nil -> ErrorResult UnexpectedEof
  --    (h:.t) -> if f h then Result t h else ErrorResult (UnexpectedChar h)
  --  )
  
--bindParser (\c -> bool (unexpectedCharParser c) (valueParser c) (p c)) character
-- c is a context (argument) so we lift it
  bindParser (bool <$> unexpectedCharParser <*> valueParser <*> p) character
...
natural ::
  Parser Int
natural =
  bindParser (\ds -> case read ds of
    Empty -> failed
    Full i -> valueParser i
    ) (list1 digit)
  --bindParser (\ds -> (??) (read ds) failed (\i->valueParser i)) (list1 digit)
...
sequenceParser (h:.t) = bindParser (\a -> mapParser (a :.) (sequenceParser t)) h
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
--thisMany 0 _ = valueParser Nil
--thisMany n p = 
  --bindParser (\a -> 
  --mapParser (\as -> a:.as) (thisMany (n-1) p)
  --) p
thisMany n p =
...
firstNameParser =
  --(:.) <$> upper <*> list lower
  (<*>) ((<$>) (:.) upper) (list lower)
  --bindParser (\c -> mapParser (\cs -> c :. cs) (list lower)) upper
  --do 
  --  u <- upper
  --  v <- list lower
  --  pure (u :. v)
