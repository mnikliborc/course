...
filter _ Nil =
  Nil
filter f (h :. hs) =
  --(if f h then (:. ) h else id) filter f hs
  bool id ((:.) h) (f h) (filter f hs)
...
filterr ::
  ...
  -> List a
  -> List a
filterr f = foldRight (\h r -> if f h then h :. r else r) Nil  
...
(++) (x:.xs) (ys) = x :. xs ++ ys 
foldRight f init xs
exchange :. (cons) of xs with f and Nil of xs with init
-}
